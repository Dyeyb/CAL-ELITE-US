...
if (!$input || empty($input['subject']) || empty($input['body'])) {
    out(false, 'Subject and body are required.', null, 400);
}
...
if (empty($recipients)) {
    out(false, 'No recipients provided from the frontend.', null, 404);
}

// Perform the actual "Blast" by sending an email
$type = strtoupper(trim($input['type'] ?? 'ANNOUNCEMENT'));
$subject = $input['subject'];
$finalSubject = "[CAL ELITE - $type] $subject";
...
<style>
  ...
  .type-badge { display: inline-block; background-color: #D4AF37; color: #000; padding: 4px 10px; border-radius: 4px; font-size: 11px; font-weight: bold; margin-bottom: 15px; letter-spacing: 1px; }
</style>
</head>
<body>
  <div class="email-wrapper">
    ...
    <div class="footer">
      ...
      This is an automated ' . strtolower($type) . ' email. Please do not reply directly to this message.
    </div>
  </div>
</body>
</html>';
...
if (class_exists('PHPMailer\PHPMailer\PHPMailer')) {
    ...
    try {
        ...
        $mail->Password = 'mryiwllxmlcnvshd'; // Removing spaces from app password
        ...
        $mail->send();
        out(true, 'Email blast sent successfully to ' . count($recipients) . ' recipients.');
    } catch (Exception $e) {
        out(false, 'Email blast failed to send (PHPMailer error: ' . $mail->ErrorInfo . ')', null, 500);
    }
} else {
    ...
    if ($mailSuccess) {
        out(true, 'Email blast sent successfully to ' . count($recipients) . ' recipients (via basic mail).');
    } else {
        out(true, '[MOCK] Email blast processed for ' . count($recipients) . ' recipients. (mail() function failed, likely no local SMTP).');
    }
}
